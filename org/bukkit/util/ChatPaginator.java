...
/*  49 */     String[] selectedLines = (String[])Java15Compat.Arrays_copyOfRange(lines, from, to);
/*     */     
...
/*  76 */     List<String> lines = new LinkedList();
...
/*  91 */         int i; int j; if ((line.length() == 0) && (word.length() > lineLength)) {
/*  92 */           i = (arrayOfString = word.toString().split("(?<=\\G.{" + lineLength + "})")).length; for (j = 0; j < i; j++) { String partialWord = arrayOfString[j];
...
/* 101 */           i = (arrayOfString = word.toString().split("(?<=\\G.{" + lineLength + "})")).length; for (j = 0; j < i; j++) { String partialWord = arrayOfString[j];
...
/* 132 */       String pLine = (String)lines.get(i - 1);
/* 133 */       String subLine = (String)lines.get(i);
/*     */       
...
/*     */     }
/*     */     
/* 141 */     return (String[])lines.toArray(new String[lines.size()]);
/*     */   }
/*     */   
...
/* Location:              C:\Users\USUARIO\Desktop\spigot-1.8.8-R0.1-SNAPSHOT-latest.jar!\org\bukkit\util\ChatPaginator.class
