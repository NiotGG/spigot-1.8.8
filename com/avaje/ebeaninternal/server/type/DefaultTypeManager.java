...
/*     */   
/* 137 */   private final List<ScalarType<?>> customScalarTypes = new ArrayList();
/*     */   
...
/*     */     
/* 156 */     this.compoundTypeMap = new ConcurrentHashMap();
/* 157 */     this.typeMap = new ConcurrentHashMap();
/* 158 */     this.nativeMap = new ConcurrentHashMap();
/*     */     
...
/*     */     
/* 213 */     return createCompoundScalarDataReader(compoundTypeClass, compoundType, " using reflection");
/*     */   }
/*     */   
/*     */ 
...
/* 251 */     return (CtCompoundType)this.compoundTypeMap.get(type);
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/*     */   {
/* 258 */     return (ScalarType)this.nativeMap.get(Integer.valueOf(jdbcType));
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/*     */   {
/* 266 */     return (ScalarType)this.typeMap.get(type);
/*     */   }
/*     */   
...
/* 276 */       ScalarType<?> customScalarType = (ScalarType)this.customScalarTypes.get(i);
/*     */       
...
/* 289 */     ScalarDataReader<?> reader = (ScalarDataReader)this.typeMap.get(type);
/* 290 */     if (reader == null) {
/* 291 */       reader = (ScalarDataReader)this.compoundTypeMap.get(type);
/*     */     }
/* 293 */     return reader;
/*     */   }
/*     */   
...
/*     */     {
/* 312 */       return scalarType;
/*     */     }
/*     */     
/* 315 */     scalarType = (ScalarType)this.typeMap.get(type);
/* 316 */     if ((scalarType != null) && (
/* 317 */       (jdbcType == 0) || (scalarType.getJdbcType() == jdbcType)))
/*     */     {
/* 319 */       return scalarType;
/*     */     }
/*     */     
/*     */ 
/*     */ 
...
/*     */ 
/*     */ 
/* 327 */     if (type.equals(java.util.Date.class)) {
/* 328 */       return this.extraTypeFactory.createUtilDate(jdbcType);
/*     */     }
/*     */     
/* 331 */     if (type.equals(Calendar.class)) {
/* 332 */       return this.extraTypeFactory.createCalendar(jdbcType);
/*     */     }
/*     */     
...
/* 362 */       return null;
/*     */     }
/* 364 */     ScalarType<?> type = (ScalarType)this.nativeMap.get(Integer.valueOf(toJdbcType));
/* 365 */     if (type != null) {
...
/*     */     
/* 391 */     Map<String, String> nameValueMap = new HashMap();
/*     */     
/* 393 */     Field[] fields = enumType.getDeclaredFields();
/* 394 */     for (int i = 0; i < fields.length; i++) {
/* 395 */       EnumValue enumValue = (EnumValue)fields[i].getAnnotation(EnumValue.class);
...
/*     */   {
/* 424 */     EnumMapping enumMapping = (EnumMapping)enumType.getAnnotation(EnumMapping.class);
...
/*     */     
/* 450 */     Iterator it = nameValueMap.entrySet().iterator();
/* 451 */     while (it.hasNext()) {
/* 452 */       Map.Entry entry = (Map.Entry)it.next();
...
/* 484 */       Class<?> cls = (Class)foundTypes.get(i);
/*     */       try
/*     */       {
...
/* 505 */       Class<?> cls = (Class)foundTypes.get(i);
/*     */       try
/*     */       {
...
/* 522 */         ScalarTypeWrapper stw = new ScalarTypeWrapper(logicalType, wrappedType, converter);
/*     */         
...
/*     */     {
/* 541 */       Class<?> type = (Class)compoundTypes.get(j);
/*     */       try
/*     */       {
...
/*     */   {
/* 564 */     CtCompoundType<?> ctCompoundType = (CtCompoundType)this.compoundTypeMap.get(compoundTypeClass);
/* 565 */     if (ctCompoundType != null) {
/* 566 */       logger.info("Already registered compound type " + compoundTypeClass);
/* 567 */       return ctCompoundType;
/*     */     }
/*     */     
/* 570 */     CompoundTypeProperty<?, ?>[] cprops = compoundType.getProperties();
/*     */     
/* 572 */     ScalarDataReader[] dataReaders = new ScalarDataReader[cprops.length];
/*     */     
/* 574 */     for (int i = 0; i < cprops.length; i++)
/*     */     {
/* 576 */       Class<?> propertyType = getCompoundPropertyType(cprops[i]);
/*     */       
/* 578 */       ScalarDataReader<?> scalarDataReader = getScalarDataReader(propertyType, cprops[i].getDbType());
...
/* 638 */     this.typeMap.put(java.util.Date.class, utilDateType);
/*     */     
...
/* 648 */     this.typeMap.put(Boolean.TYPE, booleanType);
/*     */     
/*     */ 
...
/* 671 */     this.typeMap.put(Character.TYPE, this.charType);
...
/*     */     }
/*     */     else {
/* 684 */       ScalarType<?> platClobScalarType = (ScalarType)this.nativeMap.get(Integer.valueOf(platformClobType));
...
/*     */     }
/*     */     else {
/* 701 */       ScalarType<?> platBlobScalarType = (ScalarType)this.nativeMap.get(Integer.valueOf(platformBlobType));
...
/* 731 */     this.typeMap.put(Float.TYPE, this.floatType);
...
/* 741 */     this.typeMap.put(java.sql.Date.class, this.dateType);
...
/* Location:              C:\Users\USUARIO\Desktop\spigot-1.8.8-R0.1-SNAPSHOT-latest.jar!\com\avaje\ebeaninternal\server\type\DefaultTypeManager.class
