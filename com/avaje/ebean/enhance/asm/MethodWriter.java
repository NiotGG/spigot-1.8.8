...
/*      */       }
/*      */     }
/*  442 */     this.compute = (computeMaxs ? 1 : computeFrames ? 0 : 2);
...
/*      */       case 3: 
/*  612 */         if (delta < 64) {
/*  613 */           this.stackMap.putByte(delta);
/*      */         } else {
...
/*      */     }
/*      */     
/*  720 */     if ((var < 4) && (opcode != 169)) { int opt;
/*      */       int opt;
/*  722 */       if (opcode < 54)
/*      */       {
...
/* 1457 */         return n << 2 | ((car == 'D') || (car == 'J') ? 2 : car == 'V' ? 0 : 1);
/*      */       }
/* 1459 */       if (car == 'L') {
...
/*      */     
/*      */ 
/* 1530 */     for (int i = 0; i < locals.length; i++) {
/* 1531 */       int t = locals[i];
/* 1532 */       if (t == 16777216) {
/* 1533 */         nTop++;
/*      */       } else {
...
/*      */   {
/* 1611 */     int clocalsSize = this.frame[1];
...
/* 1622 */     int k = 0;
/*      */     int delta;
/* 1624 */     int delta; if (this.frameCount == 0) {
...
/*      */     
/* 2254 */     int u = 0;
/* 2255 */     int label; int newOffset; while (u < this.code.length) {
/* 2256 */       int opcode = b[u] & 0xFF;
/* 2257 */       int v; int j; switch (ClassWriter.TYPE[opcode]) {
/*      */       case 0: 
/*      */       case 4: 
...
/* 2324 */       case 14: case 16: case 1: case 3: case 10: case 2: case 5: case 6: case 11: case 12: case 7: case 15: default:  while (j > 0) {
/* 2325 */           label = v + readInt(b, u);
/* 2326 */           u += 4;
/* 2327 */           newOffset = getNewOffset(allIndexes, allSizes, v, label);
/* 2328 */           newCode.putInt(newOffset);j--; continue;
/*      */           
/*      */ 
/*      */ 
/*      */ 
...
/* 2351 */             newCode.putInt(newOffset);j--; continue;
/*      */             
/*      */ 
/*      */ 
...
/*      */ 
/* 2408 */           u = l.position - 3;
/* 2409 */           if (((l.status & 0x20) != 0) || ((u >= 0) && (resize[u] != 0)))
/*      */           {
...
/* 2455 */           newOffset = getNewOffset(allIndexes, allSizes, 0, label) - newOffset;
/*      */           
...
/* 2464 */       u = 0;
/* 2465 */       while (u < this.lineNumber.length) {
/* 2466 */         writeShort(b, u, getNewOffset(allIndexes, allSizes, 0, readUnsignedShort(b, u)));
/*      */         
/*      */ 
/*      */ 
/* 2470 */         u += 4;
...
/* Location:              C:\Users\USUARIO\Desktop\spigot-1.8.8-R0.1-SNAPSHOT-latest.jar!\com\avaje\ebean\enhance\asm\MethodWriter.class
