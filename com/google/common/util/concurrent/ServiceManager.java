...
/* 127 */   private static final ListenerCallQueue.Callback<Listener> HEALTHY_CALLBACK = new ListenerCallQueue.Callback("healthy()") {
/*     */     void call(ServiceManager.Listener listener) {
/* 129 */       listener.healthy();
/*     */     }
/*     */   };
/* 132 */   private static final ListenerCallQueue.Callback<Listener> STOPPED_CALLBACK = new ListenerCallQueue.Callback("stopped()") {
/*     */     void call(ServiceManager.Listener listener) {
...
/* 203 */     WeakReference<ServiceManagerState> stateReference = new WeakReference(this.state);
/*     */     
...
/*     */   }
/*     */   
/* 408 */   private static final class ServiceManagerState { final Monitor monitor = new Monitor();
/*     */     @GuardedBy("monitor")
/* 410 */     final SetMultimap<Service.State, Service> servicesByState = Multimaps.newSetMultimap(new EnumMap(Service.State.class), new Supplier()
/*     */     {
/*     */ 
...
/*     */     
/*     */     @GuardedBy("monitor")
/* 468 */     final List<ListenerCallQueue<ServiceManager.Listener>> listeners = Collections.synchronizedList(new ArrayList());
/*     */     
/*     */ 
/*     */ 
/*     */ 
...
/* 587 */       try { loadTimes = Lists.newArrayListWithCapacity(this.states.size() - this.states.count(Service.State.NEW) + this.states.count(Service.State.STARTING));
/*     */         
...
/*     */       }
/* 604 */       Collections.sort(loadTimes, Ordering.natural().onResultOf(new Function()
/*     */       {
...
/* 687 */       new ListenerCallQueue.Callback("failed({service=" + service + "})")
/*     */       {
/* 689 */         void call(ServiceManager.Listener listener) { listener.failure(service); } }.enqueueOn(this.listeners);
/*     */     }
/*     */     
/*     */ 
/*     */ 
...
/* Location:              C:\Users\USUARIO\Desktop\spigot-1.8.8-R0.1-SNAPSHOT-latest.jar!\com\google\common\util\concurrent\ServiceManager.class
