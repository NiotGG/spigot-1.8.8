...
/*  192 */   private boolean colDecimalNeedsBump = false;
...
/*      */ 
/*      */ 
/*  205 */   private boolean platformDbCharsetMatches = true;
...
/*  215 */   private boolean useNewUpdateCounts = false;
...
/*      */     
/*  281 */     this.reusablePacket = new Buffer(1024);
/*  282 */     this.sendPacket = new Buffer(1024);
/*      */     
/*  284 */     this.port = port;
/*  285 */     this.host = host;
/*      */     
...
/*      */     try
/*      */     {
/*  292 */       this.mysqlConnection = this.socketFactory.connect(this.host, this.port, props);
/*      */       
/*      */ 
/*      */ 
...
/*  307 */         this.mysqlInput = new ReadAheadInputStream(this.mysqlConnection.getInputStream(), 16384, this.connection.getTraceProtocol(), this.connection.getLog());
/*      */ 
/*      */       }
/*  310 */       else if (this.connection.useUnbufferedInput()) {
/*  311 */         this.mysqlInput = this.mysqlConnection.getInputStream();
/*      */       } else {
/*  313 */         this.mysqlInput = new BufferedInputStream(this.mysqlConnection.getInputStream(), 16384);
/*      */       }
/*      */       
/*      */ 
/*  317 */       this.mysqlOutput = new BufferedOutputStream(this.mysqlConnection.getOutputStream(), 16384);
/*      */       
/*      */ 
/*      */ 
...
/*      */       
/*  326 */       this.needToGrabQueryFromPacket = ((this.profileSql) || (this.logSlowQueries) || (this.autoGenerateTestcaseScript));
/*      */       
/*      */ 
/*      */ 
/*  330 */       if ((this.connection.getUseNanosForElapsedTime()) && (Util.nanoTimeAvailable()))
/*      */       {
...
/*      */       }
/*      */     } catch (IOException ioEx) {
/*  343 */       throw SQLError.createCommunicationsException(this.connection, 0L, 0L, ioEx, getExceptionInterceptor());
/*      */     }
/*      */   }
/*      */   
/*      */ 
...
/*      */ 
/*      */ 
/*  459 */         ResultSetImpl rs = buildResultSetWithRows(callingStatement, catalog, fields, rows, resultSetType, resultSetConcurrency, isBinaryEncoded);
/*      */         
/*      */ 
/*      */ 
/*      */ 
...
/*      */       
/*  725 */       int tableNameStart = packet.getPosition() + 1;
/*  726 */       int tableNameLength = packet.fastSkipLenString();
/*  727 */       tableNameStart = adjustStartForFieldLength(tableNameStart, tableNameLength);
/*      */       
/*      */ 
...
/*      */       
/*  738 */       nameStart = adjustStartForFieldLength(nameStart, nameLength);
/*      */       
/*      */ 
...
/*      */       
/*  749 */       long colLength = 0L;
/*      */       
/*  751 */       if (this.has41NewNewProt) {
/*  752 */         colLength = packet.readLong();
/*      */       } else {
/*  754 */         colLength = packet.readLongInt();
/*      */       }
/*      */       
/*  757 */       int colType = packet.readByte() & 0xFF;
/*      */       
/*  759 */       short colFlag = 0;
/*      */       
/*  761 */       if (this.hasLongColumnInfo) {
/*  762 */         colFlag = (short)packet.readInt();
/*      */       } else {
/*  764 */         colFlag = (short)(packet.readByte() & 0xFF);
/*      */       }
/*      */       
/*  767 */       int colDecimals = packet.readByte() & 0xFF;
/*      */       
...
/*      */       }
/*      */       
/*  777 */       Field field = new Field(this.connection, packet.getByteBuffer(), databaseNameStart, databaseNameLength, tableNameStart, tableNameLength, originalTableNameStart, originalTableNameLength, nameStart, nameLength, originalColumnNameStart, originalColumnNameLength, colLength, colType, colFlag, colDecimals, defaultValueStart, defaultValueLength, charSetNumber);
/*      */       
/*      */ 
/*      */ 
/*      */ 
...
/*      */       
/*  982 */       Iterator ringBufIter = this.packetDebugRingBuffer.iterator();
/*  983 */       while (ringBufIter.hasNext()) {
/*  984 */         dumpBuffer.append((StringBuffer)ringBufIter.next());
...
/*      */       try
/*      */       {
/* 1008 */         stmt = (PreparedStatement)this.connection.clientPrepareStatement("EXPLAIN ?");
...
/* 1260 */         this.clientParam |= 0x200;
/* 1261 */         this.has41NewNewProt = true;
/*      */         
/*      */ 
/* 1264 */         this.clientParam |= 0x2000;
/*      */         
/*      */ 
/* 1267 */         this.clientParam |= 0x20000;
/*      */         
/*      */ 
/*      */ 
/*      */ 
/* 1272 */         if (this.connection.getAllowMultiQueries()) {
/* 1273 */           this.clientParam |= 0x10000;
/*      */         }
/*      */       } else {
/* 1276 */         this.clientParam |= 0x4000;
...
/*      */         
/* 1305 */         if ((this.clientParam & 0x4000) != 0L) {
...
/*      */         
/*      */         case 254: 
/* 1625 */           len = (int)(this.mysqlInput.read() & 0xFF | (this.mysqlInput.read() & 0xFF) << 8 | (this.mysqlInput.read() & 0xFF) << 16 | (this.mysqlInput.read() & 0xFF) << 24 | (this.mysqlInput.read() & 0xFF) << 32 | (this.mysqlInput.read() & 0xFF) << 40 | (this.mysqlInput.read() & 0xFF) << 48 | (this.mysqlInput.read() & 0xFF) << 56);
/*      */           
/*      */ 
/*      */ 
/*      */ 
...
/*      */       }
/*      */       
/* 1689 */       Buffer packet = new Buffer(6);
/* 1690 */       this.packetSequence = -1;
/* 1691 */       packet.writeByte((byte)1);
/* 1692 */       send(packet, packet.getPosition());
/*      */     } finally {
/* 1694 */       forceClose();
/*      */     }
/*      */   }
...
/*      */         
/* 1745 */         java.sql.Statement owningStatement = addingTo.getStatement();
/*      */         
...
/*      */     
/* 1791 */     boolean checkForMoreResults = (this.clientParam & 0x20000) != 0L;
/*      */     
/*      */ 
...
/*      */   
/*      */ 
/* 2005 */   private int statementExecutionDepth = 0;
/*      */   private boolean useAutoSlowLog;
/*      */   
/*      */   protected boolean shouldIntercept() {
/* 2009 */     return this.statementInterceptors != null;
/*      */   }
/*      */   
/*      */ 
/*      */ 
...
/*      */     throws Exception
/*      */   {
/* 2036 */     this.statementExecutionDepth += 1;
/*      */     try
/*      */     {
...
/* 2308 */         interceptedResults = invokeStatementInterceptorsPost(query, callingStatement, rs, false, null);
/*      */         
/*      */ 
...
/* 2326 */             SQLException cause = null;
/*      */             
/* 2328 */             if (callingStatement.wasCancelledByTimeout) {
/* 2329 */               cause = new MySQLTimeoutException();
/*      */             } else {
/* 2331 */               cause = new MySQLStatementCancelledException();
/*      */             }
/*      */             
...
/* 2341 */       throw sqlEx;
/*      */     } finally {
/* 2343 */       this.statementExecutionDepth -= 1;
/*      */     }
/*      */   }
/*      */   
...
/*      */     throws SQLException
/*      */   {
/* 2385 */     Iterator interceptors = this.statementInterceptors.iterator();
/*      */     
...
/*      */       break;
/*      */     case 1008: 
/* 2640 */       rs = ResultSetImpl.getInstance(catalog, fields, rows, this.connection, callingStatement, true);
/*      */       
/*      */ 
/* 2643 */       break;
/*      */     
/*      */     default: 
/* 2646 */       return ResultSetImpl.getInstance(catalog, fields, rows, this.connection, callingStatement, false);
/*      */     }
/*      */     
/*      */     
...
/* 3591 */           throw new MysqlDataTruncation(errorBuf.toString(), 0, true, false, 0, 0, errno);
/*      */         }
...
/*      */     
/* 4029 */     byte savePacketSequence = this.packetSequence++;
/*      */     
...
/*      */ 
/*      */ 
/* 4038 */       savePacketSequence = (byte)(savePacketSequence + 1);this.packetSequence = savePacketSequence;
/* 4039 */       packet.clear();
/*      */       
...
/*      */     
/*      */     case 1: 
/* 4122 */       unpackedRowData[columnIndex] = { binaryData.readByte() };
/* 4123 */       break;
/*      */     
/*      */ 
/*      */     case 2: 
...
/*      */     int length;
/* 4199 */     int hour; int minute; int seconds; int year; int month; int day; int after1000; int after100; switch (curField.getMysqlType())
/*      */     {
/*      */     case 6: 
/*      */       break;
/*      */     
/*      */     case 1: 
/* 4205 */       byte tinyVal = binaryData.readByte();
/*      */       
...
/*      */     case 2: 
/*      */     case 13: 
/* 4222 */       short shortVal = (short)binaryData.readInt();
/*      */       
...
/*      */ 
/*      */     case 8: 
/* 4255 */       long longVal = binaryData.readLongLong();
/*      */       
...
/*      */ 
/*      */     case 4: 
/* 4271 */       float floatVal = Float.intBitsToFloat(binaryData.readIntAsLong());
/*      */       
...
/*      */ 
/*      */     case 5: 
/* 4279 */       double doubleVal = Double.longBitsToDouble(binaryData.readLongLong());
/*      */       
...
/*      */       
/*      */ 
/* 4306 */       byte[] timeAsBytes = new byte[8];
/*      */       
...
/*      */         
/* 4509 */         for (int j = 0; j < nanosAsBytes.length; j++) {
...
/* 4564 */       this.clientParam |= 0x8000;
/* 4565 */       doSecureAuth = true;
/*      */     }
/*      */     
/* 4568 */     this.clientParam |= 0x800;
/*      */     
...
/* Location:              C:\Users\USUARIO\Desktop\spigot-1.8.8-R0.1-SNAPSHOT-latest.jar!\com\mysql\jdbc\MysqlIO.class
