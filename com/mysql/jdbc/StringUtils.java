...
/*   50 */   private static byte[] allBytes = new byte['Ā'];
/*      */   
/*   52 */   private static char[] byteToChars = new char['Ā'];
/*      */   
...
/*      */     
/*   72 */     for (int i = 0; 
/*   73 */         (i < 255) && (i < allBytesStringLen); i++) {
/*   74 */       byteToChars[i] = allBytesString.charAt(i);
/*      */     }
/*      */     try
/*      */     {
...
/*  415 */       return escapeEasternUnicodeByteStream(b, s, 0, s.length());
/*      */ 
/*      */ 
/*      */     }
...
/*      */       } else {
/*  445 */         s = new String(c, offset, length);
/*      */         
...
/*  459 */       return escapeEasternUnicodeByteStream(b, s, offset, length);
/*      */ 
/*      */ 
/*      */     }
/*      */     catch (UnsupportedEncodingException uee)
...
/*  534 */       return escapeEasternUnicodeByteStream(b, s, 0, s.length());
/*      */ 
/*      */ 
/*      */     }
/*      */     catch (UnsupportedEncodingException uee)
...
/*  576 */       return escapeEasternUnicodeByteStream(b, s, 0, s.length());
/*      */ 
/*      */ 
/*      */     }
/*      */     catch (UnsupportedEncodingException uee)
...
/*  643 */       return escapeEasternUnicodeByteStream(b, s, offset, length);
/*      */ 
/*      */ 
/*      */     }
/*      */     catch (UnsupportedEncodingException uee)
...
/*  736 */         c = (char)(c - '0');
/*  737 */       } else { if (!Character.isLetter(c)) break;
/*  738 */         c = (char)(Character.toUpperCase(c) - 'A' + 10);
/*      */       }
/*      */       
/*      */ 
/*      */ 
...
/*  817 */         c = (char)(c - '0');
/*  818 */       } else { if (!Character.isLetter(c)) break;
/*  819 */         c = (char)(Character.toUpperCase(c) - 'A' + 10);
/*      */       }
/*      */       
/*      */ 
/*      */ 
...
/*  890 */         c = (char)(c - '0');
/*  891 */       } else { if (!Character.isLetter(c)) break;
/*  892 */         c = (char)(Character.toUpperCase(c) - 'A' + 10);
/*      */       }
/*      */       
/*      */ 
/*      */ 
...
/*      */         do
/*      */         {
/*  947 */           i++; } while ((i <= stopSearchingAt) && (isNotEqualIgnoreCharCase(searchIn, firstCharOfPatternUc, firstCharOfPatternLc, i)));
/*      */       }
/*      */       
/*      */ 
...
/* 1003 */       } else if ((contextMarker != 0) && (c == markerCloses.charAt(markerTypeFound)) && (!escaped)) {
/* 1004 */         contextMarker = '\000';
/* 1005 */       } else if (((ind = marker.indexOf(c)) != -1) && (!escaped) && (contextMarker == 0))
/*      */       {
/* 1007 */         markerTypeFound = ind;
/* 1008 */         contextMarker = c;
/* 1009 */       } else if (((Character.toUpperCase(c) == Character.toUpperCase(target.charAt(0))) || (Character.toLowerCase(c) == Character.toLowerCase(target.charAt(0)))) && (!escaped) && (contextMarker == 0))
/*      */       {
/*      */ 
...
/* 1034 */         contextMarker = '\000';
/* 1035 */       } else if ((c == quoteChar) && (!escaped) && (contextMarker == 0))
/*      */       {
...
/*      */     
/* 1123 */     List<String> splitTokens = new ArrayList();
/*      */     
/*      */ 
...
/* 1265 */     for (; 
/* 1265 */         beginPos < inLength; beginPos++) {
/* 1266 */       if (!Character.isWhitespace(searchIn.charAt(beginPos))) {
/*      */         break;
/*      */       }
/*      */     }
/*      */     
...
/* 1409 */           searchInPos++;
/*      */           
/* 1411 */           searchForPos++; } while ((searchForPos < searchForEnd) && (searchForWildcard.charAt(searchForPos) == wildcardOne));
/*      */         
...
/* 1428 */         for (; 
/*      */             
/* 1428 */             searchForPos != searchForEnd; searchForPos++) {
/* 1429 */           if (searchForWildcard.charAt(searchForPos) != wildcardMany)
/*      */           {
/*      */ 
/*      */ 
/* 1433 */             if (searchForWildcard.charAt(searchForPos) != wildcardOne) break;
...
/*      */         do
/*      */         {
/* 1463 */           while ((searchInPos != searchInEnd) && (Character.toUpperCase(searchIn.charAt(searchInPos)) != Character.toUpperCase(cmp)))
/*      */           {
/*      */ 
/* 1466 */             searchInPos++;
/*      */           }
...
/*      */           
/*      */         }
/* 1480 */         while ((searchInPos != searchInEnd) && (searchForWildcard.charAt(0) != wildcardMany));
/*      */         
/* 1482 */         return -1;
/*      */       }
/*      */     }
...
/*      */     try
/*      */     {
/* 1601 */       while ((currentChar = sourceReader.read()) != -1)
/*      */       {
/*      */ 
/*      */ 
...
/* 1619 */             int prevChar = 0;
/*      */             
/* 1621 */             while (((currentChar = sourceReader.read()) != 47) || (prevChar != 42)) {
...
/*      */           {
/*      */             continue;
/* 1644 */             if ((contextMarker == 0) && (currentChar == 35) && (hashComments)) {}
/*      */             
/*      */             for (;;)
/*      */             {
/* 1648 */               if (((currentChar = sourceReader.read()) != 10) && (currentChar != 13) && (currentChar >= 0)) {
/*      */                 continue;
/* 1650 */                 if ((contextMarker == 0) && (currentChar == 45) && (dashDashComments))
/*      */                 {
...
/* 1655 */                     buf.append('-');
/*      */                     
/* 1657 */                     if (currentChar == -1) break;
/* 1658 */                     buf.append(currentChar); break;
/*      */                   }
/*      */                   
/*      */ 
/*      */ 
...
/* Location:              C:\Users\USUARIO\Desktop\spigot-1.8.8-R0.1-SNAPSHOT-latest.jar!\com\mysql\jdbc\StringUtils.class
